...
- Edit the `fstab` to mount home on boot:
  ...
  UUID=<NVMe-UUID>     /home                 ext4           defaults
  ```
...
sudo pip3 install -U rosinstall_generator wstool
```
...
**Note**: Do not use `rosdep`, since it try to install dependcies via built-in packages (like `apt` in Ubuntu). I tried with rosdep and it throw a `python3-ros-catkin-pkg-modules` not found in apt. 
...
Install realsense-ros, [official turtorial](https://github.com/IntelRealSense/realsense-ros). Though `ddynamic_reconfigure` only up to `kinetic` branch, it succed in `noetic`.
...
cd ..
./src/catkin/bin/catkin_make_isolated --install -DCMAKE_BUILD_TYPE=Release --pkg ddynamic_reconfigure realsense2_camera realsense2_description
```

Install other dependencies.
```shell
sudo pip3 install -U netifaces
```
